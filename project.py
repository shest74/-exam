...
class Main(tk.Frame):
    ...
    def init_main(self):
        ...
        btn_open_dialog = tk.Button(toolbar, bg='white', bd=0, image=self.add_img, command=self.open_dialog)
        ...
        # Кнопка изменения
        self.update_img = tk.PhotoImage(file="Project/img/change.png")
        btn_edit_dialog = tk.Button(toolbar, bg="#d7d7d7",
        ...
        btn_delete = tk.Button(toolbar, bg="#d7d7d7",
                            bd = 0, image=self.delete_img,
                            command=self.delete_records)
        btn_delete.pack(side=tk.LEFT)

        # Кнопка поиска
        self.img_search = tk.PhotoImage(file="Project/img/search.png")
        ...
        # Кнопка обновления
        self.img_refresh = tk.PhotoImage(file="Project/img/refresh.png")
...
class Update(Child):
    ...
    # Инициализируем виджеты дочернего окна для изменения данных
    def init_edit(self):
        self.title("Редактировать позицию")
        btn_upd = ttk.Button(self, text="Редактировать")
        btn_upd.place(x=205, y=170)
        btn_upd.bind("<Button-1>",
                     lambda event: self.view.update_record(self.entry_name.get(),
                                                        self.entry_tel.get(),
                                                        self.entry_email.get()))
        btn_upd.bind("<Button-1>", lambda event: self.destroy(), add="+")
        self.btn_ok.destroy()
...
# класс дочернего окна для поиска данных
class Search(tk.Toplevel):
    ...
    # Инициализируем виджеты дочернего окна для поиска данных
